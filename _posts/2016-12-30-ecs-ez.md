...
Those are a lot of pieces and it might be confusing how they come together other than the small isolated examples I gave so here's a couple more involved examples that should helper paint a more concrete idea of how the architecture behaves in practice.

#### Creating a villager in Ripple

* in short you make a call to `entityManager.createEntityFromAssemblage` and pass in the assemblage for villager
* this ends up making repeated calls to `entityManager.addComponentToEntity` with each component defined in the assemblage
* each of those calls then creates a new component and then maps it to the entity’s id. in code this is just a simple hash map: components[entity_id] = entity’s component
* so after all these calls we have the state for a villager instantiated across the different components that make it up.
* but as we said earlier, there are default values for each of these components and a lot of them end up being null and assemblage data solves this problem so where does that come in
* pretty simply. we fetch each component for the villager, grab the villager’s assemblage data, and then merge all the values. this makes sure villagers have the proper specific component values instead of the default ones. in the current code if we didn’t do this, villagers would actually end up looking and behaving like zombies (literally, like zombie monsters)
* ok that solves the issue of villagers having default villager state/components but what if we want to be more specific like spawn a villager with a different colored hair or something
* same thing as the assemblage data, you would just pass in a map of component states to overwrite the defaults and then we use those. currently this is actually how loading the game works. you just spawn agents and buildings like normal except you pass in whatever their current state and that’s what they get loaded with
