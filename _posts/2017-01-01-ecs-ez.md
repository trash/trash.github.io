...
I didn't like any of these solutions so I tried to give "composition over inheritance" a go. The first thing I isolated was the storage functionality. If a class needed to store things then it would have a storage property, `entity.storage`, which would be an instance of the `Storage` class. This class would encapsulate all of the functionality around storing items and could be attached to any other entity class.

At this point I started to convert pieces of functionality over to this format, where classes like my `Agent` and `Building` classes started having shared functionality pulled out into separate classes and attached as properties. While I had composable classes at this point they were still tied into the original object-oriented class hierarchy and all the problems that entailed. I realized by doing this I was essentially implementing a half-assed version of ECS. Bored and frustrated, I decided to go whole-ass with my solution and move everything to ECS.
...
To be more explicit, the core parts of the ECS framework are <small>surprise</small> Entities, Components, and Systems. And in ECS, this id **is** the entity. Components are raw data representing one piece of functionality for entities (`Position`, `Health`, `Storage`) and are tied to each entity by its id. Systems, which are aligned to a specific type of Component, operate on each Component of their type every turn (`StorageSystem` operates on `StorageComponents`).
...
// entity's health due to the `update` method above
...
Systems have methods to perform updates on components but this is only within the context of the System's `update` method. What happens if we want to update a component from outside the System? Or what if a System needs to update a component not of its given type? This is where Utils come in. They are tied to a Component of a certain type similar to Systems, but provide publicly accessible methods for performing updates on that component type. They exist as a good location to consolidate logic relevant to a certain Component without tying it to a System, whose methods can't be shared.

One use-case for Utils is for the use of more complicated data structures for your Components' state. Ideally Components should be made up of primitive data structures <small>[1]</small> and therefore, since they have no methods attached to them, if you want to use more advanced data structures you'll need a place to store that logic outside the component itself. Utils are a good spot for this code.
...
When I initially heard about ECS I thought it sounded a bit too complicated. Why invest in building or integrating a fairly substantial framework and opinionated architecture when objects "just work". Well an object-oriented approach did "just work" until it "totally did not work". While the object-oriented approach was very fast to get up and running with, the long-term costs in time spent refactoring and rewriting code started to add up till adding certain features became a seemingly insurmountable endeavor. It got to the point where I put off features I absolutely should have been working on just because the thought of breaking yet another core system left me apathetic.

When I finally bit the bullet with ECS I was surprised with how quickly I was able to get up and running with a basic implementation. After that it was just a matter of expanding and making changes as I went. This post has a lot of writing but I think overall the architecture is grounded on common sense and principles that developer's are familiar with. It's those properties that made me stick with it.
