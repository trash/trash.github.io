...
An example from Ripple's development was when I was thinking about adding a `StorageCart` class to the game. I already had a `Building` class which encompassed the relevant functionality like storing items inside the building and having the building be constructed. So this seems like a good class to inherit from because a storage cart does both of these things. But a storage cart is slightly different in that it also can move, whereas a building cannot. Well buildings can't move but I had an `Agent` class that did have this behavior.

At this point, I had three choices on how to implement the `StorageCart` class:
...
2. Have the `StorageCart` class extend the `Building` class. So the `StorageCart` class ends up sharing most of the `Building` functionality but then has to duplicate the movement-handling code from the `Agent` class. Again, not great because there's still duplicated code.
    <div class="small-image">
...
The first pieces of the puzzle are entities, the most simple concept to grapple with. An entity in ECS is an id with a number of components tied to it. When I say a component is tied to an entity's id I just mean, given the id, you can fetch any component for that entity. Think of it as a primary key in a database.
...
What happens if we want to modify a component outside the context of a System whose `update` method is called every turn? This is where Utils come in. They are tied to a Component of a certain type similar to Systems, and provide publicly accessible methods for performing updates on that component type. If we had two pieces of code that needed to perform the same update on a `MoveComponent` for example, they would use the `MoveUtil`.

Another use for Utils is for using more complicated data structures for your Components' state. Ideally Components should be made up of primitive data structures <small>[1]</small> and therefore, since they have no methods attached to them, if you want to use more advanced data structures you'll need a place to store that logic outside the component itself. Utils are a good spot for this functionality.
